...
## Usage
  ...
  - **mvn clean install**
    ...
    - Use to install the dependencies specified in pom.xml
- **mvn package**
...
- **Running individual tests**
  ...
  - Inside test files specify the Recipient CM account address instead of "CM_ACCOUNT_ADDRESS"

  - Specify the Distributor's url under `grpc.client.cmb-client.addres` in [./src/main/resources/application.properties](./src/main/resources/application.properties)
    e.g. if you are running the distributor locally on port 9090 `grpc.client.cmb-client.address=localhost:9090`

  - Run these commands to run individual tests

  ```
  mvn -Dtest=com.chain4travel.cmbplugin.GrpcClientTransportSearchServiceTest test
  mvn -Dtest=com.chain4travel.cmbplugin.GrpcClientPingServiceTest test
  ```
